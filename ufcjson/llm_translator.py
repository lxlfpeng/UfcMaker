...
def translate_many(values, cache_conn=None, unanswered=None):
    """批量翻译。返回 {原文: 译文}；未配置/失败时返回空 dict。

    values 内重复项会自动去重，空串忽略。
    cache_conn: 可选的翻译缓存库连接；传入则每批成功即写缓存，
    即使中途中断也不会丢已完成的批次。
    unanswered: 可选的 set，用于回传「这一批请求成功了、但模型没给出译文」的原文
    （返回空串或被漏答）。调用方据此记账、累计到上限后搁置，避免每天重发同一批专有名词。
    ⚠️ 整批硬失败（网络/接口异常，tr is None）**不算**在 unanswered 里——那是暂时故障，
    下轮自然重试；混进来会让一次故障就把整批活儿记成「模型答不出」。
    """
    ...
    for i in range(0, len(values), batch_size):
        ...
        if unanswered is not None:
            unanswered.update(k for k, v in tr.items() if not v)
        result.update(tr)
