...
# 页面战绩区常有 '<br>+零宽空格' 这种空文本节点，会变成 '\u200b' 这样的垃圾条目
# 混进 history，进而卡住 translator 的整行回填。判定函数见 ufcjson/textutil.py
# （与 translator、.workbuddy/scripts/fix_translate_dates.py 共用同一份定义，
#  三者必须口径一致，否则 history / history_cn 会按位错位）。
...
class AthleteSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item in items:
            ...
            self.cursor.execute("SELECT * FROM player WHERE page = ? AND record = ?", (player['page'], player['record']))
